...
namespace WpfControlsX.ControlX
{
    ...
    /// Author      : BigWang
    /// Created Time: 2023/4/15 22:13:54
    /// Description :
    /// ----------------------------------------------------------------
    /// Version      Modified Time              Modified By     Modified Content
    /// V1.0.0.0     2023/4/15 22:13:54                     BigWang         首次编写         
    ///
    public class WxCoverView : RegularItemsControl
    {
        private readonly WxCoverViewContent _viewContent;

        private WxCoverViewItem _selectedItem;

        private IEnumerable _itemsSourceInternal;

        private readonly Dictionary<object, WxCoverViewItem> _entryDic = new();

        private bool _isRefresh;

        public WxCoverView()
        {
            _viewContent = new WxCoverViewContent();

            AddHandler(SelectableItem.SelectedEvent, new RoutedEventHandler(CoverViewItem_OnSelected));
            _viewContent.SetBinding(WxCoverViewContent.ContentHeightProperty, new Binding(ItemContentHeightProperty.Name) { Source = this });
        ...
        {
            if (e.OriginalSource is WxCoverViewItem item)
            {
        ...
        protected override bool IsItemItsOwnContainerOverride(object item) => item is WxCoverViewItem;

        public static readonly DependencyProperty CoverViewContentStyleProperty = DependencyProperty.Register(
            nameof(CoverViewContentStyle), typeof(Style), typeof(WxCoverView), new PropertyMetadata(default(Style)));
        ...
        internal static readonly DependencyProperty GroupsProperty = DependencyProperty.Register(
            nameof(Groups), typeof(int), typeof(WxCoverView),
        ...
        public static readonly DependencyProperty ItemContentHeightProperty = DependencyProperty.Register(
            nameof(ItemContentHeight), typeof(double), typeof(WxCoverView), new PropertyMetadata(ValueBoxes.Double300Box),
        ...
        public static readonly DependencyProperty ItemContentHeightFixedProperty = DependencyProperty.Register(
            nameof(ItemContentHeightFixed), typeof(bool), typeof(WxCoverView), new PropertyMetadata(ValueBoxes.TrueBox));
        ...
        public static readonly DependencyProperty ItemHeaderTemplateProperty = DependencyProperty.Register(
            nameof(ItemHeaderTemplate), typeof(DataTemplate), typeof(WxCoverView), new PropertyMetadata(default(DataTemplate)));
        ...
        private void UpdateCoverViewContentPosition()
        {
            ...
            {
                int row = _selectedItem.Index / Groups + 1;
                if (row == totalRow)
                {
                    ItemsHost.Children.Add(_viewContent);
                }
        ...
        private void InsertItem(int index, object item)
        {
            ...
            else
            {
                ...
                if (container is WxCoverViewItem element)
                {
